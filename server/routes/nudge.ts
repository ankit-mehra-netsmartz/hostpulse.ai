...
async function generateAIResponse(
  ...
  try {
    ...
    const systemPrompt = campaign.aiInstructions || 
...
async function analyzeFeedback(messages: any[]): Promise<{ summary: string; sentiment: string }> {
...
export function registerNudgeRoutes(app: Express, storage: IStorage) {
      ...
      const campaignsWithStats = await Promise.all(
        campaigns.map(async (campaign) => {
          ...
          return {
            ...
            positiveCount: conversations.filter(c => c.sentiment === 'positive').length,
            neutralCount: conversations.filter(c => c.sentiment === 'neutral').length,
            negativeCount: conversations.filter(c => c.sentiment === 'negative').length,
          };
        })
      );
  ...
  app.patch("/api/nudge/campaigns/:id", async (req, res) => {
    ...
    try {
      ...
      const hasAccess = await validateWorkspaceMembership(userId, existingCampaign.workspaceId);
      if (!hasAccess) {
  ...
  app.get("/api/nudge/conversations", async (req, res) => {
    ...
    try {
      ...
      const hasAccess = await validateWorkspaceMembership(userId, campaign.workspaceId);
      if (!hasAccess) {
        return res.status(403).json({ error: "Access denied" });
      }
  ...
  app.get("/api/nudge/conversations/:id", async (req, res) => {
    ...
    try {
      ...
      const hasAccess = await validateWorkspaceMembership(userId, conversation.workspaceId);
      if (!hasAccess) {
        return res.status(403).json({ error: "Access denied" });
      }
  ...
  app.post("/api/nudge/conversations", async (req, res) => {
    ...
    try {
      const userId = getUserId(req);
      const { campaignId, guestName, guestPhone, listingId, listingName, workspaceId } = req.body;
      ...
      if (campaign.workspaceId !== workspaceId) {
        return res.status(403).json({ error: "Campaign does not belong to this workspace" });
      }
      ...
      let initialMessage = campaign.initialMessage
        ...
        .replace(/%checkout_date/g, new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric' }))
  ...
  app.post("/api/webhooks/twilio/sms", async (req, res) => {
    try {
      ...
      if (!conversation) {
        logger.info('Twilio Webhook', 'No active conversation found for:', From);
      ...
      if (optOutKeywords.some(kw => Body.toLowerCase().includes(kw))) {
        ...
        // Send confirmation
        await sendSMS(From, "You have been unsubscribed. Thank you for your feedback!");
      ...
      if (outboundCount >= maxMessages) {
        ...
        await sendSMS(From, "Thank you so much for sharing your feedback! We really appreciate it and will use it to improve your future stays.");
