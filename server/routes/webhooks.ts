...
const hospitableReservationWebhookSchema = z.object({
  ...
  action: z.enum(["reservation.created", "reservation.changed", "reservation.cancelled"]),
  data: z.object({
    id: z.string(),
    conversation_id: z.string().optional(),
    platform: z.enum(["airbnb", "homeaway", "booking", "direct", "manual"]).optional(),
  ...
  triggers: z.array(z.enum([
    ...
    "checkin_changed", "checkout_changed", "financials_changed", 
    "guest_issue_detected", "test", "historic_reservation"
  ])).optional(),
...
const hospitablePropertyWebhookSchema = z.object({
  action: z.enum(["property.created", "property.changed", "property.deleted", "property.merged"]),
  data: z.object({
    property: z.object({
      id: z.string(),
      name: z.string().optional(),
...
const hospitableMessageWebhookSchema = z.object({
  id: z.string().optional(),
  action: z.enum(["message.created", "message.updated", "message.sent", "message.received"]),
  data: z.object({
    platform: z.string().optional(),
...
export function registerWebhookRoutes(app: Express, storage: IStorage) {
  app.post("/api/webhooks/hospitable", async (req, res) => {
    ...
    try {
      ...
      if (!webhookSecret) {
        const secretName = config.isDevelopment ? 'HOSPITABLE_WEBHOOK_SECRET_DEV' : 'HOSPITABLE_WEBHOOK_SECRET';
      ...
      switch (event) {
        ...
        case "reservation.cancelled": {
          const parseResult = hospitableReservationWebhookSchema.safeParse(req.body);
          if (!parseResult.success) {
            logger.error("Webhook", "Invalid reservation webhook payload:", parseResult.error);
            await storage.createWebhookLog({
              source: "hospitable",
              eventType: event,
              status: "error",
            ...
            return res.status(400).json({ message: "Invalid reservation webhook payload" });
          }

          const resData = parseResult.data.data;
          logReservationId = resData.id;

          const existingReservation = await storage.findReservationByExternalId(resData.id);
          ...
          logger.info("Webhook", `Reservation webhook: ${event}, triggers: ${triggers.join(', ') || 'none'}`);

          let derivedStatus: string;
          if (event === "reservation.cancelled") {
            derivedStatus = "cancelled";
          } else {
            derivedStatus = resData.reservation_status?.current?.category || resData.status || "confirmed";
          }
          ...
          if (existingReservation) {
            const updateData: any = {
              ...
              confirmationCode: confirmationCode || existingReservation.confirmationCode,
            };
            ...
            if (resData.guest) {
              const guestName = [resData.guest.first_name, resData.guest.last_name].filter(Boolean).join(' ');
              if (guestName) updateData.guestName = guestName;
              if (resData.guest.email) updateData.guestEmail = resData.guest.email;
              const guestPic = resData.guest.profile_picture || resData.guest.picture_url;
              if (guestPic) updateData.guestProfilePicture = guestPic;
              if (resData.guest.location) updateData.guestLocation = resData.guest.location;
            }
            ...
            if (resData.platform) {
              updateData.platform = resData.platform.charAt(0).toUpperCase() + resData.platform.slice(1);
            }

            await storage.updateReservation(existingReservation.id, updateData);
            logger.info("Webhook", `Updated reservation: ${existingReservation.id} (status: ${derivedStatus}, event: ${event}, triggers: ${triggers.join(', ')})`);

            const acceptedStatuses = ["accepted", "confirmed", "new"];
            if (listing && listing.workspaceId && acceptedStatuses.includes(derivedStatus) && checkOutDate) {
              try {
                const existingTask = await storage.findExistingCleaningTask(
                  listing.workspaceId, listing.id, existingReservation.id
                );
                if (!existingTask) {
                  const assignments = await storage.getAssignmentsByListing(listing.id);
                  const activeAssignments = assignments.filter(a => a.isActive);
                  if (activeAssignments.length > 0) {
                    const assignment = activeAssignments[0];
                    const accessToken = crypto.randomBytes(32).toString('hex');
                    const guestName = resData.guest
                      ? [resData.guest.first_name, resData.guest.last_name].filter(Boolean).join(' ')
                    ...
                    if (assignment.assignmentMode === "auto" && assignment.defaultMemberId) {
                      webhookTaskData.assignedMemberId = assignment.defaultMemberId;
                    }
                    const task = await storage.createCleaningTask(webhookTaskData);

                    if (webhookProcedureId) {
                      const procedureWithSteps = await storage.getProcedureWithSteps(webhookProcedureId);
                    ...
                    logger.info("Webhook", `Auto-generated cleaning task ${task.id} for reservation ${existingReservation.id} via webhook`);
                  }
                }
              } catch (autoGenError) {
                logger.error("Webhook", "Error auto-generating cleaning task from webhook:", autoGenError);
              }
            }

            try {
              const cleaningTasksForRes = await storage.getCleaningTasksByReservationId(existingReservation.id);
              if (cleaningTasksForRes.length > 0 && listing) {
                ...
                for (const ct of cleaningTasksForRes) {
                  ...
                  const scheduledDate = new Date(ct.scheduledDate).toLocaleDateString('en-US', {
                    weekday: 'long', month: 'long', day: 'numeric'
                  });
                  const shortCodeData = {
                    property_name: ct.listing.name,
                    address: ct.listing.address || '',
                    check_in_date: checkInDate ? checkInDate.toLocaleDateString('en-US', { month: 'long', day: 'numeric' }) : '',
                    check_out_date: checkOutDate ? checkOutDate.toLocaleDateString('en-US', { month: 'long', day: 'numeric' }) : scheduledDate,
                  ...
                  if (event === "reservation.cancelled") {
                    await storage.updateCleaningTask(ct.id, { status: "cancelled" });
                    const emailTmpl = await storage.getNotificationTemplate(ct.workspaceId, "cancelled_email");
                    const smsTmpl = await storage.getNotificationTemplate(ct.workspaceId, "cancelled_sms");

                    if (cleaner.notifyByEmail && cleaner.email) {
                      const tmpl = emailTmpl || DEFAULT_TEMPLATES.cancelled_email;
                      await sendTemplatedEmail({
                        toEmail: cleaner.email,
                        subject: tmpl.subject || DEFAULT_TEMPLATES.cancelled_email.subject,
                        body: tmpl.body,
                        shortCodeData,
                      }).catch(e => logger.error("Webhook", "Failed to send cancellation email:", e));
                    }
                    if (cleaner.notifyBySms && cleaner.phone) {
                      try {
                        ...
                        await sendSMS(cleaner.phone, renderTemplate(tmpl.body, shortCodeData));
                      } catch (e) { logger.error("Webhook", "Failed to send cancellation SMS:", e); }
                    }
                    logger.info("Webhook", `Notified cleaner ${cleaner.name} about cancelled reservation ${existingReservation.id}`);
                    ...
                    if (checkOutDate) {
                      ...
                      if (newScheduledDate.toDateString() !== oldScheduledDate.toDateString()) {
                        await storage.updateCleaningTask(ct.id, {
                          scheduledDate: newScheduledDate,
                          reminderSentAt: null,
                        });
                        shortCodeData.scheduled_date = newScheduledDate.toLocaleDateString('en-US', {
                          weekday: 'long', month: 'long', day: 'numeric'
                        });
                      }
                    }

                    const emailTmpl = await storage.getNotificationTemplate(ct.workspaceId, "changed_email");
                    const smsTmpl = await storage.getNotificationTemplate(ct.workspaceId, "changed_sms");

                    if (cleaner.notifyByEmail && cleaner.email) {
                      const tmpl = emailTmpl || DEFAULT_TEMPLATES.changed_email;
                      await sendTemplatedEmail({
                        toEmail: cleaner.email,
                        subject: tmpl.subject || DEFAULT_TEMPLATES.changed_email.subject,
                        body: tmpl.body,
                        shortCodeData,
                      }).catch(e => logger.error("Webhook", "Failed to send change email:", e));
                    }
                    if (cleaner.notifyBySms && cleaner.phone) {
                      try {
                        const { sendSMS } = await import('../services/twilio');
                        ...
                        await sendSMS(cleaner.phone, renderTemplate(tmpl.body, shortCodeData));
                      } catch (e) { logger.error("Webhook", "Failed to send change SMS:", e); }
                    }
                    logger.info("Webhook", `Notified cleaner ${cleaner.name} about changed reservation ${existingReservation.id}`);
                  }
                }
              }
            } catch (notifyError) {
              logger.error("Webhook", "Error notifying cleaners about reservation change:", notifyError);
            }
          } else if (listing && event !== "reservation.cancelled") {
            const guestName = resData.guest 
              ? [resData.guest.first_name, resData.guest.last_name].filter(Boolean).join(' ') || resData.guest.first_name
              : "Unknown Guest";
            
            const platform = resData.platform 
              ? resData.platform.charAt(0).toUpperCase() + resData.platform.slice(1)
              : "Airbnb";
            ...
            const newReservation = await storage.createReservation({
              ...
              guestProfilePicture: resData.guest?.profile_picture || resData.guest?.picture_url || null,
            ...
            logger.info("Webhook", `Created reservation: ${newReservation.id} from webhook (guest: ${guestName}, platform: ${platform})`);
            logReservationId = newReservation.id;

            const newResAcceptedStatuses = ["accepted", "confirmed", "new"];
            if (checkOutDate && listing.workspaceId && newResAcceptedStatuses.includes(derivedStatus)) {
              try {
                const assignments = await storage.getAssignmentsByListing(listing.id);
                const activeAssignments = assignments.filter(a => a.isActive);
                if (activeAssignments.length > 0) {
                  ...
                  if (assignment.assignmentMode === "auto" && assignment.defaultMemberId) {
                    newResTaskData.assignedMemberId = assignment.defaultMemberId;
                  }
                  const task = await storage.createCleaningTask(newResTaskData);

                  if (assignment.procedureId) {
                    const procedureWithSteps = await storage.getProcedureWithSteps(assignment.procedureId);
                    if (procedureWithSteps && procedureWithSteps.steps.length > 0) {
                      const items = procedureWithSteps.steps.map(step => ({
                        cleaningTaskId: task.id,
                        stepOrder: step.stepOrder,
                  ...
                  logger.info("Webhook", `Auto-generated cleaning task ${task.id} for new reservation ${newReservation.id} via webhook`);
                }
              } catch (autoGenError) {
                logger.error("Webhook", "Error auto-generating cleaning task for new reservation:", autoGenError);
              }
            }
          } else {
            logger.info("Webhook", `Skipped reservation webhook - listing not found for property: ${propertyId || listingPlatformId}`);
          }
          ...
          return res.status(200).json({ message: `Reservation ${event.split('.')[1]} processed` });
        }
        ...
        case "property.merged": {
          const parseResult = hospitablePropertyWebhookSchema.safeParse(req.body);
          if (!parseResult.success) {
            logger.error("Webhook", "Invalid property webhook payload:", parseResult.error);
            await storage.createWebhookLog({
              source: "hospitable",
              eventType: event,
              status: "error",
            ...
            return res.status(400).json({ message: "Invalid property webhook payload" });
          }
          ...
          if (event === "property.created") {
            if (existingListing) {
              ...
              logger.info("Webhook", `Property ${propertyId} marked for sync review (already exists)`);
            } else if (property) {
              ...
              if (targetWorkspaceId && targetUserId) {
                ...
                const airbnbListing = property.listings?.find(l => l.platform === "airbnb");
                ...
                logger.info("Webhook", `New property ${propertyId} created with pending_sync status for user ${targetUserId}`);
              } else {
                logger.info("Webhook", `New property ${propertyId} received but no matching user found for email: ${userEmail}`);
              }
            }
            ...
            logger.info("Webhook", `Property ${propertyId} has updates pending user review`);
          } else if (event === "property.deleted" && existingListing) {
            await storage.updateListing(existingListing.id, {
              webhookStatus: "pending_delete",
              webhookPendingData: req.body,
            });
            logger.info("Webhook", `Property ${propertyId} marked as pending deletion - user action required`);
          } else if (event === "property.merged" && existingListing) {
            await storage.updateListing(existingListing.id, {
              webhookStatus: "pending_merge",
              webhookPendingData: { ...req.body, merged_into },
            });
            logger.info("Webhook", `Property ${propertyId} marked as merged into ${merged_into} - user action required`);
          }
          ...
          return res.status(200).json({ message: `Property ${event.split('.')[1]} processed` });
        }
        ...
        case "review.created": {
          ...
          if (!parseResult.success) {
            logger.error("Webhook", "Invalid review webhook payload:", parseResult.error);
            await storage.createWebhookLog({
              source: "hospitable",
              eventType: event,
              status: "error",
            ...
            return res.status(400).json({ message: "Invalid review webhook payload" });
          }

          const { reservation_id, review } = parseResult.data.data;
          logReservationId = reservation_id;
          
          const reservation = await storage.findReservationByExternalId(reservation_id);
          if (!reservation) {
            logger.info("Webhook", `Reservation not found for external ID: ${reservation_id}`);
            await storage.createWebhookLog({
              source: "hospitable",
              eventType: event,
              status: "not_found",
          ...
          await storage.updateReservation(reservation.id, {
            publicReview: review?.public?.review || review?.public_review || reservation.publicReview,
            privateRemarks: review?.private?.feedback || review?.private_remarks || reservation.privateRemarks,
            guestRating: review?.public?.rating ?? review?.rating ?? reservation.guestRating,
            categoryRatings: categoryRatingsData as any,
            hostReply: review?.public?.response || review?.host_reply || reservation.hostReply,
            reviewPostedAt: review?.reviewed_at ? new Date(review.reviewed_at) : new Date(),
          });

          logger.info("Webhook", `Review applied to reservation: ${reservation.id}`);

          try {
            const listing = await storage.getListing(reservation.listingId);
            if (listing) {
              const updatedReservation = await storage.getReservation(reservation.id);
              if (updatedReservation) {
                const conversationText = updatedReservation.conversationHistory
...
Respond in JSON format:
{
  ...
  "aiGuestSummary": "string"
                ...
                logger.info("Webhook", `AI Sentiment analysis completed for reservation: ${reservation.id}`);
              }
            }
          } catch (aiError) {
            logger.error("Webhook", `Failed to run AI analysis for reservation ${reservation.id}:`, aiError);
          }
        ...
        case "message.received": {
          const parseResult = hospitableMessageWebhookSchema.safeParse(req.body);
          if (!parseResult.success) {
            logger.error("Webhook", "Invalid message webhook payload:", parseResult.error);
            await storage.createWebhookLog({
              source: "hospitable",
              eventType: event,
              status: "error",
            ...
            return res.status(400).json({ message: "Invalid message webhook payload" });
          }

          const msgData = parseResult.data.data;
          logReservationId = msgData.reservation_id || undefined;

          const propertyId = msgData.property?.id != null ? String(msgData.property.id) : null;
          const listingPlatformId = msgData.listing?.platform_id != null ? String(msgData.listing.platform_id) : null;
          const platformMsgId = msgData.platform_id != null ? Number(msgData.platform_id) : undefined;
          ...
          if (!reservation && msgData.conversation_id) {
            reservation = await storage.findReservationByExternalId(msgData.conversation_id);
          }
          ...
          if (!reservation && (propertyId || listingPlatformId)) {
            ...
            if (matchedListing) {
              ...
              const listingReservations = await storage.getReservationsByListing(matchedListing.id);
              if (listingReservations.length > 0) {
                ...
                if (reservation) {
                  logger.info("Webhook", `Message matched to reservation ${reservation.id} via listing ${matchedListing.id}`);
                }
              } else {
                logger.info("Webhook", `Message linked to listing ${matchedListing.id} (no reservations found - inquiry message)`);
              }
            }
          }
          ...
          if (reservation) {
            ...
            const existingHistory = (reservation.conversationHistory || []) as Array<{
              id: string;
              sender: "guest" | "host";
              timestamp: string;
              message: string;
            ...
            const normalizedSender: "guest" | "host" = msgData.sender_type === "guest" ? "guest" : "host";
            
            const existingMsgIndex = platformMsgId 
              ? existingHistory.findIndex(m => m.platform_id === platformMsgId)
              : -1;
            
            if (event === "message.updated" && existingMsgIndex >= 0) {
              existingHistory[existingMsgIndex] = {
                ...existingHistory[existingMsgIndex],
                message: msgData.body || existingHistory[existingMsgIndex].message,
                timestamp: msgData.created_at || existingHistory[existingMsgIndex].timestamp,
              };
              
              await storage.updateReservation(reservation.id, {
                conversationHistory: existingHistory as any,
              });
              logger.info("Webhook", `Message updated in reservation ${reservation.id} conversation history`);
            } else if (platformMsgId && existingHistory.some(m => m.platform_id === platformMsgId)) {
              logger.info("Webhook", `Message ${platformMsgId} already exists in reservation ${reservation.id}, skipping duplicate`);
            } else {
              const newMessage = {
              ...
              logger.info("Webhook", `Message added to reservation ${reservation.id} conversation history (sender: ${normalizedSender})`);
            }
          } else {
            logger.info("Webhook", `Skipped message - no matching reservation. reservation_id: ${msgData.reservation_id || 'null'}, conversation_id: ${msgData.conversation_id || 'null'}, property: ${propertyId || 'null'}`);
          }
