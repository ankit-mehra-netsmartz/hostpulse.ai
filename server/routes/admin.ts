...
import {
  USER_ROLES, AI_MODELS, type AIModelId, type DataSource,
  procedures, cleaners, workspaces,
} from "@shared/schema";
import { logger } from "../logger";
import { getUserId, getParamId } from "./helpers";
import { openai, openrouter } from "./ai-helpers";

const isAppAdmin = async (userId: string, storage: IStorage): Promise<boolean> => {
  const user = await storage.getUser(userId);
  return user?.role === USER_ROLES.APP_ADMIN;
};

const hasAdminAccess = async (userId: string, storage: IStorage): Promise<boolean> => {
  const user = await storage.getUser(userId);
  return user?.role === USER_ROLES.APP_ADMIN || user?.role === USER_ROLES.ADMIN_USER;
};

async function generateChangelogSuggestions(storage: IStorage, userId?: string): Promise<{ suggestions: any[]; message: string; latestCommit?: string }> {
  ...
  let commits: { hash: string; fullHash: string; message: string; date: string }[] = [];
  ...
  try {
    ...
    if (lastProcessedCommit) {
      ...
      gitCommand = 'git log --since="7 days ago" --pretty=format:"%H|||%s|||%ai" --no-merges 2>/dev/null || echo ""';
    }
    ...
    logger.info("Changelog Suggest", "Git command failed, may not be in a git repository:", gitError);
  }

  if (commits.length === 0) {
    return { suggestions: [], message: lastProcessedCommit ? "No new commits since last analysis" : "No recent commits found" };
  }

  const existingEntries = await storage.getChangelogEntries();
  const existingHashes = new Set(existingEntries.map(e => e.commitHash).filter(Boolean));
  ...
  if (newCommits.length === 0) {
    ...
    return { suggestions: [], message: "All recent commits have already been processed" };
  }
...
If there are no user-facing changes worth mentioning, return: { "entries": [] }`;
  ...
  const response = await openai.chat.completions.create({
    model: "gpt-4.1-mini",
    messages: [
      { role: "system", content: "You are a product changelog writer who creates fun, host-friendly update notes. Always respond with valid JSON." },
...
export function scheduleChangelogSuggest(storage: IStorage) {
  if (changelogSuggestScheduled) {
    logger.info("Changelog Suggest", "Already scheduled, skipping duplicate registration");
    return;
  }
  ...
  const runCheck = async () => {
    try {
      ...
      const currentMinute = easternTime.getMinutes().toString().padStart(2, "0");
      ...
      logger.info("Changelog Suggest", `Running scheduled suggestion generation at ${currentHour}:${currentMinute} ET`);

      try {
        const result = await generateChangelogSuggestions(storage);
        logger.info("Changelog Suggest", result.message);
      } catch (err) {
        logger.error("Changelog Suggest", "Failed to generate suggestions:", err);
      }
    } catch (error) {
  ...
  logger.info("Changelog Suggest", "Scheduled to check every minute for configured suggest time");
}
...
export function scheduleChangelogSend(storage: IStorage) {
  if (changelogSendScheduled) {
    logger.info("Changelog Scheduler", "Already scheduled, skipping duplicate registration");
    return;
  }
  ...
  const runCheck = async () => {
    try {
      ...
      if (settings.lastSentAt) {
        ...
        const lastSentEastern = new Date(lastSent.toLocaleString("en-US", { timeZone: "America/New_York" }));
        if (lastSentEastern.toDateString() === easternTime.toDateString()) {
          return;
        }
      }

      logger.info("Changelog Scheduler", "Running scheduled send at", currentTime, "ET");

      const approvedEntries = await storage.getChangelogEntriesByStatus("approved");
        ...
        for (const user of users) {
          if (user.email) {
            try {
              ...
              logger.error("Changelog Scheduler", `Failed to send email to ${user.email}:`, err);
            }
          }
        }
      }
      ...
      if (notificationType === "in_app" || notificationType === "both") {
        logger.info("Changelog Scheduler", "In-app notifications would be sent here");
      }
      ...
      logger.info("Changelog Scheduler", `Sent ${approvedEntries.length} changelog entries`);
    } catch (error) {
  ...
  logger.info("Changelog Scheduler", "Scheduled to check every minute for configured send time");
}
...
export function registerAdminRoutes(app: Express, storage: IStorage): void {
  ...
  app.get("/api/admin/procedure-template", isAuthenticated, async (req, res) => {
    try {
      const userId = getUserId(req);
      if (!await isAppAdmin(userId, storage)) {
        return res.status(403).json({ message: "Access denied. Application Admin required." });
      }
  ...
  app.put("/api/admin/procedure-template", isAuthenticated, async (req, res) => {
    try {
      const userId = getUserId(req);
      if (!await isAppAdmin(userId, storage)) {
        return res.status(403).json({ message: "Access denied. Application Admin required." });
  ...
  app.post("/api/admin/procedure-template/import/:procedureId", isAuthenticated, async (req, res) => {
    try {
      const userId = getUserId(req);
      if (!await isAppAdmin(userId, storage)) {
        return res.status(403).json({ message: "Access denied. Application Admin required." });
      ...
      res.status(500).json({ message: "Failed to import procedure as template" });
    }
  });
  ...
  app.get("/api/admin/all-procedures", isAuthenticated, async (req, res) => {
    try {
      ...
      const allProcs = await db.select({
        id: procedures.id,
        title: procedures.title,
        status: procedures.status,
        workspaceId: procedures.workspaceId,
      }).from(procedures).orderBy(procedures.title);

      const result = await Promise.all(allProcs.map(async (p) => {
  ...
  app.get("/api/admin/prompts/category/:category", isAuthenticated, async (req, res) => {
    try {
      const userId = getUserId(req);
      if (!await isAppAdmin(userId, storage)) {
        return res.status(403).json({ message: "Access denied. Application Admin required." });
      ...
      const prompt = await storage.getPromptByCategory(getParamId(req.params.category));
      if (!prompt) {
        return res.status(404).json({ message: "Prompt not found for category" });
      }
      res.json(prompt);
    } catch (error) {
  ...
  app.post("/api/admin/prompts/seed", isAuthenticated, async (req, res) => {
    try {
      ...
      res.json({ message: "Default prompts seeded successfully", count: prompts.length });
    } catch (error) {
  ...
  app.get("/api/admin/ai-model", isAuthenticated, async (req, res) => {
    try {
      ...
      res.json({ model: currentModel, updatedAt: setting?.updatedAt, updatedBy: setting?.updatedBy });
    } catch (error) {
  ...
  app.post("/api/admin/ai-model", isAuthenticated, async (req, res) => {
    try {
      ...
      const modelSchema = z.object({
        ...
        updatePrompts: z.enum(["using-default", "using-old-default", "none"]).optional(),
        oldModel: z.string().optional(),
      });

      const parseResult = modelSchema.safeParse(req.body);
      if (!parseResult.success) {
        return res.status(400).json({ message: "Invalid model", errors: parseResult.error.flatten() });
      }
      ...
      if (updatePrompts && updatePrompts !== "none") {
        ...
        if (updatePrompts === "using-default") {
          ...
          const promptsToUpdate = allPrompts.filter(p => p.modelId === oldModel);
          for (const prompt of promptsToUpdate) {
            await storage.updatePrompt(prompt.id, { modelId: model });
            promptsUpdated++;
          }
  ...
  app.get("/api/admin/category-weights", isAuthenticated, async (req, res) => {
    try {
      ...
      res.json({ weights, updatedAt: setting?.updatedAt, updatedBy: setting?.updatedBy });
    } catch (error) {
  ...
  app.post("/api/admin/category-weights", isAuthenticated, async (req, res) => {
    try {
      ...
      if (!parseResult.success) {
        return res.status(400).json({ message: "Invalid weights", errors: parseResult.error.flatten() });
      }
      ...
      const setting = await storage.setSystemSetting("category_weights", JSON.stringify(weights), userId);
      res.json({ weights, updatedAt: setting.updatedAt, updatedBy: setting.updatedBy });
    } catch (error) {
  ...
  app.get("/api/admin/sync-ai-model", isAuthenticated, async (req, res) => {
    try {
      ...
      if (!setting?.value) {
        ...
        return res.json({ model: fallbackModel, updatedAt: null, updatedBy: null, isDefault: true });
      }
      res.json({ model: setting.value, updatedAt: setting.updatedAt, updatedBy: setting.updatedBy, isDefault: false });
    } catch (error) {
      logger.error("Admin", "Error fetching sync AI model setting:", error);
      res.status(500).json({ message: "Failed to fetch sync AI model setting" });
    }
  });
  ...
  app.post("/api/admin/sync-ai-model", isAuthenticated, async (req, res) => {
    try {
      ...
      const setting = await storage.setSystemSetting("sync_ai_model", model, userId);
      res.json({ model: setting.value, updatedAt: setting.updatedAt, updatedBy: setting.updatedBy });
    } catch (error) {
  ...
  app.get("/api/admin/sentiment-ai-model", isAuthenticated, async (req, res) => {
    try {
      const userId = getUserId(req);
      if (!await isAppAdmin(userId, storage)) {
        return res.status(403).json({ message: "Access denied. Application Admin required." });
      ...
      logger.error("Admin", "Error fetching sentiment AI model setting:", error);
      res.status(500).json({ message: "Failed to fetch sentiment AI model setting" });
    }
  });

  app.post("/api/admin/sentiment-ai-model", isAuthenticated, async (req, res) => {
    try {
      const userId = getUserId(req);
      if (!await isAppAdmin(userId, storage)) {
        return res.status(403).json({ message: "Access denied. Application Admin required." });
      ...
      const { model } = parseResult.data;
      const setting = await storage.setSystemSetting("sentiment_ai_model", model, userId);
      res.json({ model: setting.value, updatedAt: setting.updatedAt, updatedBy: setting.updatedBy });
    } catch (error) {
  ...
  app.get("/api/admin/users", isAuthenticated, async (req, res) => {
    try {
      ...
      const allCleanerProfiles = await db.select().from(cleaners).where(isNotNull(cleaners.userId));
  ...
  app.patch("/api/admin/users/:id/role", isAuthenticated, async (req, res) => {
    try {
      ...
      const roleSchema = z.object({
        role: z.enum([USER_ROLES.APP_ADMIN, USER_ROLES.ADMIN_USER, USER_ROLES.USER_MANAGER, USER_ROLES.USER_STAFF]),
      });

      const parseResult = roleSchema.safeParse(req.body);
      if (!parseResult.success) {
        return res.status(400).json({ message: "Invalid role", errors: parseResult.error.flatten() });
      }

      const updated = await storage.updateUserRole(getParamId(req.params.id), parseResult.data.role);
      if (!updated) {
  ...
  app.patch("/api/admin/permissions", isAuthenticated, async (req, res) => {
    try {
      ...
      const permission = await storage.upsertRolePermission(role, navItemId, enabled);
  ...
  app.get("/api/user/permissions", isAuthenticated, async (req, res) => {
    try {
      ...
      if (user.role === USER_ROLES.APP_ADMIN || user.role === USER_ROLES.ADMIN_USER) {
        return res.json({
          fullAccess: true,
          disabledNavItems: []
        });
  ...
  app.get("/api/admin/changelog/settings", isAuthenticated, async (req, res) => {
    try {
      const userId = getUserId(req);
      if (!await isAppAdmin(userId, storage)) {
        return res.status(403).json({ message: "Access denied. Application Admin required." });
      ...
      const settings = await storage.getChangelogSettings();
      res.json(settings || { sendTime: "09:00", notificationType: "both", isEnabled: true });
    } catch (error) {
  ...
  app.post("/api/admin/changelog/settings", isAuthenticated, async (req, res) => {
    try {
      const userId = getUserId(req);
      if (!await isAppAdmin(userId, storage)) {
        return res.status(403).json({ message: "Access denied. Application Admin required." });
      ...
      const settingsSchema = z.object({
        sendTime: z.string().regex(/^\d{2}:\d{2}$/, "Time must be in HH:MM format"),
      ...
      if (!parseResult.success) {
        return res.status(400).json({ message: "Invalid settings", errors: parseResult.error.flatten() });
      }

      const settings = await storage.createOrUpdateChangelogSettings(parseResult.data);
      res.json(settings);
    } catch (error) {
      logger.error("Admin", "Error updating changelog settings:", error);
      res.status(500).json({ message: "Failed to update changelog settings" });
    }
  });
  ...
  app.post("/api/admin/changelog/entries", isAuthenticated, async (req, res) => {
    try {
      const userId = getUserId(req);
      if (!await isAppAdmin(userId, storage)) {
        return res.status(403).json({ message: "Access denied. Application Admin required." });
      ...
      const entrySchema = z.object({
        ...
        status: z.enum(["suggested", "approved", "sent", "dismissed"]).optional(),
      });

      const parseResult = entrySchema.safeParse(req.body);
      if (!parseResult.success) {
        return res.status(400).json({ message: "Invalid entry data", errors: parseResult.error.flatten() });
      }
  ...
  app.patch("/api/admin/changelog/entries/:id", isAuthenticated, async (req, res) => {
    try {
      const userId = getUserId(req);
      if (!await isAppAdmin(userId, storage)) {
        return res.status(403).json({ message: "Access denied. Application Admin required." });
      ...
      if (parseResult.data.approvedAt !== undefined) {
        updateData.approvedAt = parseResult.data.approvedAt ? new Date(parseResult.data.approvedAt) : null;
      }
      if (parseResult.data.sentAt !== undefined) {
        updateData.sentAt = parseResult.data.sentAt ? new Date(parseResult.data.sentAt) : null;
      }
      ...
      const entry = await storage.updateChangelogEntry(req.params.id, updateData);
      if (!entry) {
  ...
  app.delete("/api/admin/changelog/entries/:id", isAuthenticated, async (req, res) => {
    try {
      const userId = getUserId(req);
      if (!await isAppAdmin(userId, storage)) {
        return res.status(403).json({ message: "Access denied. Application Admin required." });
  ...
  app.post("/api/admin/changelog/generate-suggestions", isAuthenticated, async (req, res) => {
    try {
      const userId = getUserId(req);
      if (!await isAppAdmin(userId, storage)) {
        return res.status(403).json({ message: "Access denied. Application Admin required." });
      ...
      res.status(500).json({ message: "Failed to generate changelog suggestions" });
    }
  });
  ...
  app.get("/api/admin/impersonation-status", isAuthenticated, async (req, res) => {
    try {
      ...
      if (user.isImpersonating) {
        const impersonatedUser = await storage.getUser(user.impersonatedUserId);
        res.json({
  ...
  app.post("/api/admin/ai-speed-test", isAuthenticated, async (req, res) => {
    try {
      ...
      const testPrompt = prompt || "Analyze this short-term rental description and provide 3 improvement suggestions: 'Cozy 2BR apartment near downtown. Clean and quiet. Free parking. Great views.'";
      ...
      if (openaiModelInfo && openaiModelInfo.provider !== "openai") {
        return res.status(400).json({ message: `Model ${openaiModelId} is not an OpenAI model` });
      }
      if (grokModelInfo && grokModelInfo.provider !== "openrouter") {
        return res.status(400).json({ message: `Model ${grokModelId} is not an OpenRouter/Grok model` });
      }
      ...
      const results: {
        ...
        openai: { model: openaiModelId, responseTime: 0, tokens: { input: 0, output: 0 }, response: "" },
        grok: { model: grokModelId, responseTime: 0, tokens: { input: 0, output: 0 }, response: "" },
      };
      ...
      try {
        ...
        results.openai.response = openaiResponse.choices[0]?.message?.content || "";
      } catch (error: any) {
        results.openai.responseTime = Date.now() - openaiStart;
      ...
      const speedComparison = bothSucceeded ? {
        fasterModel: results.openai.responseTime < results.grok.responseTime ? "openai" : "grok",
        timeDifference: Math.abs(results.openai.responseTime - results.grok.responseTime),
        percentageFaster: results.openai.responseTime < results.grok.responseTime
          ? ((results.grok.responseTime - results.openai.responseTime) / results.grok.responseTime * 100).toFixed(1)
          : ((results.openai.responseTime - results.grok.responseTime) / results.openai.responseTime * 100).toFixed(1),
  ...
  app.post("/api/admin/ai-comparison-test", isAuthenticated, async (req, res) => {
    try {
      const userId = getUserId(req);
      if (!await isAppAdmin(userId, storage)) {
        return res.status(403).json({ message: "Access denied. Application Admin required." });
      ...
      const { workspaceId, listingId, startDate, endDate, openaiModel, grokModel } = parseResult.data;
      const openaiModelId = openaiModel || "gpt-4.1-mini";
      const grokModelId = grokModel || "x-ai/grok-3-mini";

      const startDateParsed = new Date(startDate);
      const endDateParsed = new Date(endDate);
      const daysBack = Math.ceil((endDateParsed.getTime() - startDateParsed.getTime()) / (1000 * 60 * 60 * 24));

      const listing = await storage.getListing(listingId);
      if (!listing || listing.workspaceId !== workspaceId) {
        return res.status(404).json({ message: "Listing not found in workspace" });
      }
      ...
      if (filteredReservations.length === 0) {
        return res.status(400).json({ message: "No reservations found in the selected date range" });
      }
      ...
      const reservationsFormatted = reservationsContext.map((r, idx) => `
...
- For NEGATIVE tags: Always include suggestedTaskTitle and suggestedTaskDescription`;
      ...
      const parseAIResponse = (responseText: string): { reservations: any[] } | null => {
        try {
      ...
      const { AI_MODELS } = await import("@shared/schema");
      const openaiMInfo = (AI_MODELS as any)[openaiModelId] || { inputCost: 0.0004, outputCost: 0.0016 };
      const grokMInfo = (AI_MODELS as any)[grokModelId] || { inputCost: 0.0003, outputCost: 0.0005 };
      ...
      const results: {
        ...
        openai: { model: openaiModelId, responseTime: 0, tokens: { input: 0, output: 0 }, estimatedCost: 0, rawResponse: "", parsedResults: null },
        grok: { model: grokModelId, responseTime: 0, tokens: { input: 0, output: 0 }, estimatedCost: 0, rawResponse: "", parsedResults: null },
      };

      logger.info("Comparison Test", `Running OpenAI (${openaiModelId}) with ${testReservations.length} reservations...`);
      const openaiStart = Date.now();
      try {
        const openaiResponse = await openai.chat.completions.create({
          model: openaiModelId,
          messages: [
            { role: "system", content: "You are an expert short-term rental consultant providing detailed analysis in JSON format." },
        ...
        results.openai.rawResponse = openaiResponse.choices[0]?.message?.content || "";
        results.openai.parsedResults = parseAIResponse(results.openai.rawResponse);
      } catch (error: any) {
        results.openai.responseTime = Date.now() - openaiStart;
        results.openai.error = error.message || "OpenAI request failed";
      }

      logger.info("Comparison Test", `Running Grok (${grokModelId}) with ${testReservations.length} reservations...`);
      const grokStart = Date.now();
      try {
        const grokResponse = await openrouter.chat.completions.create({
          model: grokModelId,
        ...
        results.grok.rawResponse = grokResponse.choices[0]?.message?.content || "";
        results.grok.parsedResults = parseAIResponse(results.grok.rawResponse);
      } catch (error: any) {
        results.grok.responseTime = Date.now() - grokStart;
        results.grok.error = error.message || "Grok request failed";
      }
      ...
      const tasksPrompt = (tags: any[]) => `Based on these Tags extracted from guest reservation data, generate actionable Tasks for the host.
  ...
  "tasks": [
    {
      ...
      const stageResults = {
        tags: {
          openai: { responseTime: results.openai.responseTime, tokens: results.openai.tokens, estimatedCost: results.openai.estimatedCost, error: results.openai.error },
          grok: { responseTime: results.grok.responseTime, tokens: results.grok.tokens, estimatedCost: results.grok.estimatedCost, error: results.grok.error },
          winner: (results.openai.responseTime < results.grok.responseTime ? "openai" : results.openai.responseTime > results.grok.responseTime ? "grok" : "tie") as "openai" | "grok" | "tie",
        },
        tasks: {
          openai: { responseTime: 0, tokens: { input: 0, output: 0 }, estimatedCost: 0, error: undefined as string | undefined },
          grok: { responseTime: 0, tokens: { input: 0, output: 0 }, estimatedCost: 0, error: undefined as string | undefined },
      ...
      const openaiTags = results.openai.parsedResults?.reservations?.flatMap((r: any) => r.tags || []) || [];
      const grokTags = results.grok.parsedResults?.reservations?.flatMap((r: any) => r.tags || []) || [];
      ...
      if (openaiTags.length > 0) {
        ...
        try {
          const openaiTasksResponse = await openai.chat.completions.create({
            model: openaiModelId,
            messages: [
              { role: "system", content: "You are an expert short-term rental consultant." },
              { role: "user", content: tasksPrompt(openaiTags) }
            ],
            max_tokens: 2000,
            response_format: { type: "json_object" },
          });
          stageResults.tasks.openai.responseTime = Date.now() - openaiTasksStart;
          ...
          stageResults.tasks.openai.estimatedCost =
            (stageResults.tasks.openai.tokens.input * openaiMInfo.inputCost / 1000) +
            (stageResults.tasks.openai.tokens.output * openaiMInfo.outputCost / 1000);
          openaiTasksResult = parseAIResponse(openaiTasksResponse.choices[0]?.message?.content || "") || { tasks: [] };
        } catch (error: any) {
          stageResults.tasks.openai.responseTime = Date.now() - openaiTasksStart;
          stageResults.tasks.openai.error = error.message || "OpenAI tasks request failed";
        }
      }
      ...
      if (grokTags.length > 0) {
        ...
        try {
          ...
          stageResults.tasks.grok.estimatedCost =
            (stageResults.tasks.grok.tokens.input * grokMInfo.inputCost / 1000) +
            (stageResults.tasks.grok.tokens.output * grokMInfo.outputCost / 1000);
          grokTasksResult = parseAIResponse(grokTasksResponse.choices[0]?.message?.content || "") || { tasks: [] };
        } catch (error: any) {
          stageResults.tasks.grok.responseTime = Date.now() - grokTasksStart;
          stageResults.tasks.grok.error = error.message || "Grok tasks request failed";
        }
      }

      if (!stageResults.tasks.openai.error && !stageResults.tasks.grok.error) {
        stageResults.tasks.winner = stageResults.tasks.openai.responseTime < stageResults.tasks.grok.responseTime ? "openai" :
          stageResults.tasks.openai.responseTime > stageResults.tasks.grok.responseTime ? "grok" : "tie";
      }

      // === STAGE 3: THEMES ===
      logger.info("Comparison Test", "Running Stage 3: Themes generation...");

      const themesPrompt = (tags: any[]) => `Analyze these Tags extracted from guest reservation data and suggest Themes to organize them.
...
Respond in JSON format:
{
  "themes": [
    {
      ...
      if (openaiTags.length > 0) {
        ...
        try {
          const openaiThemesResponse = await openai.chat.completions.create({
            model: openaiModelId,
            messages: [
              { role: "system", content: "You are an expert at categorizing and organizing feedback." },
              { role: "user", content: themesPrompt(openaiTags) }
            ],
            max_tokens: 2000,
            response_format: { type: "json_object" },
          });
          stageResults.themes.openai.responseTime = Date.now() - openaiThemesStart;
          ...
          stageResults.themes.openai.estimatedCost =
            (stageResults.themes.openai.tokens.input * openaiMInfo.inputCost / 1000) +
            (stageResults.themes.openai.tokens.output * openaiMInfo.outputCost / 1000);
          openaiThemesResult = parseAIResponse(openaiThemesResponse.choices[0]?.message?.content || "") || { themes: [] };
        } catch (error: any) {
          stageResults.themes.openai.responseTime = Date.now() - openaiThemesStart;
          stageResults.themes.openai.error = error.message || "OpenAI themes request failed";
        }
      }
      ...
      if (grokTags.length > 0) {
        ...
        try {
          ...
          stageResults.themes.grok.responseTime = Date.now() - grokThemesStart;
          ...
          stageResults.themes.grok.estimatedCost =
            (stageResults.themes.grok.tokens.input * grokMInfo.inputCost / 1000) +
            (stageResults.themes.grok.tokens.output * grokMInfo.outputCost / 1000);
          grokThemesResult = parseAIResponse(grokThemesResponse.choices[0]?.message?.content || "") || { themes: [] };
        } catch (error: any) {
          stageResults.themes.grok.responseTime = Date.now() - grokThemesStart;
          stageResults.themes.grok.error = error.message || "Grok themes request failed";
        }
      }

      if (!stageResults.themes.openai.error && !stageResults.themes.grok.error) {
        stageResults.themes.winner = stageResults.themes.openai.responseTime < stageResults.themes.grok.responseTime ? "openai" :
          stageResults.themes.openai.responseTime > stageResults.themes.grok.responseTime ? "grok" : "tie";
      }

      const totals = {
        openai: {
          responseTime: stageResults.tags.openai.responseTime + stageResults.tasks.openai.responseTime + stageResults.themes.openai.responseTime,
          tokens: {
            input: stageResults.tags.openai.tokens.input + stageResults.tasks.openai.tokens.input + stageResults.themes.openai.tokens.input,
            output: stageResults.tags.openai.tokens.output + stageResults.tasks.openai.tokens.output + stageResults.themes.openai.tokens.output,
          },
          estimatedCost: stageResults.tags.openai.estimatedCost + stageResults.tasks.openai.estimatedCost + stageResults.themes.openai.estimatedCost,
        },
        grok: {
          responseTime: stageResults.tags.grok.responseTime + stageResults.tasks.grok.responseTime + stageResults.themes.grok.responseTime,
          tokens: {
            input: stageResults.tags.grok.tokens.input + stageResults.tasks.grok.tokens.input + stageResults.themes.grok.tokens.input,
            output: stageResults.tags.grok.tokens.output + stageResults.tasks.grok.tokens.output + stageResults.themes.grok.tokens.output,
          },
          estimatedCost: stageResults.tags.grok.estimatedCost + stageResults.tasks.grok.estimatedCost + stageResults.themes.grok.estimatedCost,
        },
      };

      const overallWinner = totals.openai.responseTime < totals.grok.responseTime ? "openai" :
        totals.openai.responseTime > totals.grok.responseTime ? "grok" : "tie";

      const reservationComparisons = testReservations.map(r => {
        const openaiResult = results.openai.parsedResults?.reservations?.find((x: any) => x.reservationId === r.id);
        const grokResult = results.grok.parsedResults?.reservations?.find((x: any) => x.reservationId === r.id);
  ...
  app.get("/api/admin/speed-test-history", isAuthenticated, async (req, res) => {
    try {
      const userId = getUserId(req);
      if (!await isAppAdmin(userId, storage)) {
        return res.status(403).json({ message: "Access denied. Application Admin required." });
  ...
  app.get("/api/admin/speed-test-history/:id", isAuthenticated, async (req, res) => {
    try {
      const userId = getUserId(req);
      if (!await isAppAdmin(userId, storage)) {
        return res.status(403).json({ message: "Access denied. Application Admin required." });
  ...
  app.post("/api/admin/sync-speed-test", isAuthenticated, async (req, res) => {
    try {
      ...
      const { workspaceId, listingId, startDate, endDate, modelA, modelB } = parseResult.data;
      ...
      const filteredReservations = allReservations.filter(r => {
        ...
        const checkOut = r.checkOutDate ? new Date(r.checkOutDate) : null;
        if (!checkIn) return false;
        return checkIn <= endDateParsed && (!checkOut || checkOut >= startDateParsed);
      });
...
Each reservation MUST have at least 1 tag. Extract insights from reviews, messages, and remarks.`;
      ...
      const { AI_MODELS } = await import("@shared/schema");
      const modelAInfo = (AI_MODELS as any)[modelA] || { inputCost: 0.0004, outputCost: 0.0016, provider: 'openai' };
      const modelBInfo = (AI_MODELS as any)[modelB] || { inputCost: 0.0003, outputCost: 0.0005, provider: 'openrouter' };

      const syncResults = {
        modelA: { model: modelA, responseTime: 0, tokens: { input: 0, output: 0 }, estimatedCost: 0, tagCount: 0, error: undefined as string | undefined },
        modelB: { model: modelB, responseTime: 0, tokens: { input: 0, output: 0 }, estimatedCost: 0, tagCount: 0, error: undefined as string | undefined },
      };
      ...
      const runModel = async (modelId: string, modelInfo: any) => {
        ...
        try {
          ...
          const tokens = { input: response.usage?.prompt_tokens || 0, output: response.usage?.completion_tokens || 0 };
          const estimatedCost = (tokens.input * modelInfo.inputCost / 1000) + (tokens.output * modelInfo.outputCost / 1000);
          ...
          try {
            ...
            tagCount = parsedReservations.reduce((sum: number, r: any) => sum + (r.tags?.length || 0), 0) || 0;
            if (tagCount === 0) {
              parseError = "Response parsed but contained 0 tags - invalid test result";
            }
          } catch (e: any) {
            parseError = `JSON parse error: ${e.message}`;
          }
          return { responseTime, tokens, estimatedCost, tagCount, error: parseError, reservations: parsedReservations };
        } catch (error: any) {
          return { responseTime: Date.now() - start, tokens: { input: 0, output: 0 }, estimatedCost: 0, tagCount: 0, error: error.message, reservations: [] };
        }
      };

      logger.info("Speed Test", `Starting parallel test: ${modelA} vs ${modelB}`);
      const startTime = Date.now();
      const [resultA, resultB] = await Promise.all([
        runModel(modelA, modelAInfo),
        runModel(modelB, modelBInfo),
      ]);
      logger.info("Speed Test", `Both tests completed in ${Date.now() - startTime}ms`);

      syncResults.modelA = { model: modelA, ...resultA };
      syncResults.modelB = { model: modelB, ...resultB };

      const modelAValid = !syncResults.modelA.error && syncResults.modelA.tagCount > 0;
      const modelBValid = !syncResults.modelB.error && syncResults.modelB.tagCount > 0;
      ...
      if (!modelAValid && !modelBValid) {
        ...
        winner = syncResults.modelA.responseTime < syncResults.modelB.responseTime ? "modelA" :
                 syncResults.modelA.responseTime > syncResults.modelB.responseTime ? "modelB" : "tie";
      }

      const timeDiff = Math.abs(syncResults.modelA.responseTime - syncResults.modelB.responseTime);
      const fasterTime = Math.min(syncResults.modelA.responseTime, syncResults.modelB.responseTime);
      const percentageFaster = fasterTime > 0 ? ((timeDiff / Math.max(syncResults.modelA.responseTime, syncResults.modelB.responseTime)) * 100).toFixed(1) : "0";

      res.json({
        testType: "sync",
        testConfig: { workspaceId, listingId, listingName: listing.name, dateRange: { startDate, endDate }, reservationCount: testReservations.length },
        results: syncResults,
        comparison: { winner, timeDifference: timeDiff, percentageFaster, hasErrors },
        timestamp: new Date().toISOString(),
      });
    } catch (error) {
  ...
  app.post("/api/admin/sentiment-speed-test", isAuthenticated, async (req, res) => {
    try {
      const userId = getUserId(req);
      if (!await isAppAdmin(userId, storage)) {
        return res.status(403).json({ message: "Access denied. Application Admin required." });
...
Each reservation MUST have a score between 0 and 5 (use 0.1 increments). Score EVERY reservation.`;
      ...
      const sentimentResults = {
        modelA: { model: modelA, responseTime: 0, tokens: { input: 0, output: 0 }, estimatedCost: 0, scoresReturned: 0, error: undefined as string | undefined },
        modelB: { model: modelB, responseTime: 0, tokens: { input: 0, output: 0 }, estimatedCost: 0, scoresReturned: 0, error: undefined as string | undefined },
      };
      ...
      const runModel = async (modelId: string, modelInfo: any) => {
        ...
        try {
          ...
          try {
            ...
            const parsed = JSON.parse(cleaned);
            parsedReservations = parsed?.reservations || [];
            scoresReturned = parsedReservations.filter((r: any) => r.score !== undefined).length || 0;
            if (scoresReturned === 0) {
              parseError = "Response parsed but contained 0 scores - invalid test result";
            }
          } catch (e: any) {
            parseError = `JSON parse error: ${e.message}`;
          }
          return { responseTime, tokens, estimatedCost, scoresReturned, error: parseError, reservations: parsedReservations };
        } catch (error: any) {
          return { responseTime: Date.now() - start, tokens: { input: 0, output: 0 }, estimatedCost: 0, scoresReturned: 0, error: error.message, reservations: [] };
        }
      };

      logger.info("Sentiment Speed Test", `Starting parallel test: ${modelA} vs ${modelB}`);
      const startTime = Date.now();
      const [resultA, resultB] = await Promise.all([
        runModel(modelA, modelAInfo),
        runModel(modelB, modelBInfo),
      ]);
      logger.info("Sentiment Speed Test", `Both tests completed in ${Date.now() - startTime}ms`);

      sentimentResults.modelA = { model: modelA, ...resultA };
      sentimentResults.modelB = { model: modelB, ...resultB };

      const modelAValid = !sentimentResults.modelA.error && sentimentResults.modelA.scoresReturned > 0;
      const modelBValid = !sentimentResults.modelB.error && sentimentResults.modelB.scoresReturned > 0;
      const hasErrors = !modelAValid || !modelBValid;
      ...
      if (!modelAValid && !modelBValid) {
        ...
        winner = sentimentResults.modelA.responseTime < sentimentResults.modelB.responseTime ? "modelA" :
                 sentimentResults.modelA.responseTime > sentimentResults.modelB.responseTime ? "modelB" : "tie";
      }

      const timeDiff = Math.abs(sentimentResults.modelA.responseTime - sentimentResults.modelB.responseTime);
      const fasterTime = Math.min(sentimentResults.modelA.responseTime, sentimentResults.modelB.responseTime);
      const percentageFaster = fasterTime > 0 ? ((timeDiff / Math.max(sentimentResults.modelA.responseTime, sentimentResults.modelB.responseTime)) * 100).toFixed(1) : "0";
  ...
  app.get("/api/admin/data-sources", isAuthenticated, async (req, res) => {
    try {
      ...
      const maskedSources = allDataSources.map((ds: DataSource) => ({
        ...
        refreshToken: ds.refreshToken ? `***${ds.refreshToken.slice(-8)}` : null,
      }));
  ...
  app.get("/api/admin/data-sources/:id/export", isAuthenticated, async (req, res) => {
    try {
      const userId = getUserId(req);
      if (!await isAppAdmin(userId, storage)) {
        return res.status(403).json({ message: "Access denied. Application Admin required." });
  ...
  app.post("/api/admin/data-sources/import", isAuthenticated, async (req, res) => {
    try {
      const userId = getUserId(req);
      if (!await isAppAdmin(userId, storage)) {
        return res.status(403).json({ message: "Access denied. Application Admin required." });
      ...
      const { provider, name, accessToken, refreshToken, tokenExpiresAt, workspaceId } = parseResult.data;
      ...
      logger.info("Admin", `Data source imported: ${newDataSource.id} by user ${userId}`);
