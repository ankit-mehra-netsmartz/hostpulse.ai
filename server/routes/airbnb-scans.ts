...
export function registerAirbnbScanRoutes(app: Express, storage: IStorage) {
      ...
      try {
        ...
        if (!airbnbId) {
          return res.status(400).json({
            message:
              "No Airbnb listing ID found for this property. Please sync property data first.",
          });
        }
        ...
        (async () => {
          try {
            ...
            if (result.success) {
              ...
              logger.info(
                "AirbnbScan",
                `Scan completed for listing ${listing.id}`,
              );
            } else {
              await storage.updateAirbnbScan(scan.id, {
                status: "failed",
              ...
              logger.error(
                "AirbnbScan",
                `Scan failed for listing ${listing.id}:`,
                result.errorMessage,
              );
            }
          } catch (error) {
  ...
  app.get("/api/airbnb-scans/:id", isAuthenticated, async (req, res) => {
    try {
      ...
      if (
        scan.workspaceId &&
        !(await validateWorkspaceMembership(userId, scan.workspaceId))
      ) {
        return res.status(403).json({ message: "Not authorized" });
      }
...
${
  scan.hasWhereYoullSleep
    ? `Present: Yes
Rooms: ${JSON.stringify(scan.whereYoullSleep?.rooms || [], null, 2)}`
    : "Present: No - This section is missing from the listing"
}
...
Attributes: ${scan.hostProfile.attributes?.join(", ") || "None listed"}`
    : "Host profile data not available"
}
...
Please provide analysis in the following JSON format:
{
  ...
  "overallScore": 1-10 numeric score
          ...
          messages: [
            {
              role: "system",
              content:
                "You are an expert vacation rental analyst. Always respond with valid JSON.",
            },
        ...
        const inputCost =
          (modelInfo as any).inputCost ||
          (modelInfo as any).inputCostPer1k / 1000 ||
          0;
        const outputCost =
          (modelInfo as any).outputCost ||
          (modelInfo as any).outputCostPer1k / 1000 ||
          0;
        await storage.createAiUsageLog({
          ...
          estimatedCost:
            (response.usage?.prompt_tokens || 0) * inputCost +
