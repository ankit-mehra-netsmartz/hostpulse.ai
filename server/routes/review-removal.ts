  ...
  for (const key of Object.keys(result)) {
    if (typeof result[key] === 'string') {
      ...
      result[key] = result[key].map((item: any) => {
        ...
        if (item && typeof item === 'object') return stripMarkdownFromAnalysis(item);
        return item;
      });
    }
  }
...
async function runCaseAnalysis(
  ...
  template = template.replace("{{property_name}}", caseData.propertyName || "Unknown");
  template = template.replace("{{guest_name}}", caseData.guestName || "Unknown");
  template = template.replace("{{guest_rating}}", String(caseData.guestRating || "N/A"));
  template = template.replace("{{review_text}}", caseData.reviewText || "No review text provided");
  template = template.replace("{{category_ratings}}", JSON.stringify(caseData.categoryRatings || {}));
  template = template.replace("{{house_rules}}", caseData.houseRules || "Not provided");
  template = template.replace("{{guest_messages}}", caseData.guestMessages || "Not provided");
  template = template.replace("{{resolution_messages}}", caseData.resolutionMessages || "Not provided");
  template = template.replace("{{challenge_history}}", JSON.stringify(challengeHistory));
  ...
  const updatedCase = await storage.updateReviewRemovalCase(caseData.id, updates);
  return { updatedCase, analysis };
}

export function registerReviewRemovalRoutes(app: Express, storage: IStorage) {
  app.get("/api/review-removal/bad-reviews", async (req, res) => {
    try {
      ...
      const badReviews = allReservations.filter(r =>
        r.guestRating != null && r.guestRating < 5 && r.publicReview
      );

      const existingCases = await storage.getReviewRemovalCasesByWorkspace(workspaceId);
      const caseReservationIds = new Set(existingCases.map(c => c.reservationId));

      const reviewsWithCaseStatus = badReviews.map(r => ({
        ...r,
        hasCase: caseReservationIds.has(r.id),
        existingCase: existingCases.find(c => c.reservationId === r.id) || null,
      }));
  ...
  app.get("/api/review-removal/cases", async (req, res) => {
    try {
      const userId = getUserId(req);
      const workspaceId = getWorkspaceId(req);
      if (!workspaceId) return res.status(400).json({ message: "Workspace required" });
  ...
  app.get("/api/review-removal/cases/:id", async (req, res) => {
    try {
      const userId = getUserId(req);
      const caseData = await storage.getReviewRemovalCase(req.params.id);
      if (!caseData) return res.status(404).json({ message: "Case not found" });

      if (!await validateWorkspaceMembership(userId, caseData.workspaceId)) {
        return res.status(403).json({ message: "Not a member of this workspace" });
      }

      let reservationDetails = null;
      if (caseData.reservationId) {
        const reservation = await storage.getReservation(caseData.reservationId);
        if (reservation) {
          reservationDetails = {
  ...
  app.post("/api/review-removal/cases", async (req, res) => {
    try {
      const userId = getUserId(req);
      const workspaceId = getWorkspaceId(req);
      if (!workspaceId) return res.status(400).json({ message: "Workspace required" });
      ...
      const existing = await storage.getReviewRemovalCaseByReservation(reservationId);
      if (existing) return res.status(400).json({ message: "Case already exists for this reservation", case: existing });
      ...
      if (reservation.conversationHistory && Array.isArray(reservation.conversationHistory)) {
      ...
      try {
        const { updatedCase, analysis } = await runCaseAnalysis(storage, newCase, userId);
        res.json({ case: updatedCase, analysis, autoAnalyzed: true });
      } catch (analysisError) {
        logger.error('ReviewRemoval', 'Auto-analysis failed, returning case without analysis:', analysisError);
  ...
  app.patch("/api/review-removal/cases/:id", async (req, res) => {
    try {
      const userId = getUserId(req);
      const caseData = await storage.getReviewRemovalCase(req.params.id);
      if (!caseData) return res.status(404).json({ message: "Case not found" });
      ...
      const allowedFields = ["houseRules", "guestMessages", "resolutionMessages", "status", "stage", "challengeHistory"];
      const updates: any = {};
      ...
      if (updates.status === "won" || updates.status === "lost" || updates.status === "abandoned") {
        updates.stage = "resolved";
      }

      const updated = await storage.updateReviewRemovalCase(req.params.id, updates);
      res.json(updated);
    } catch (error) {
  ...
  app.delete("/api/review-removal/cases/:id", async (req, res) => {
    try {
      const userId = getUserId(req);
      const caseData = await storage.getReviewRemovalCase(req.params.id);
      if (!caseData) return res.status(404).json({ message: "Case not found" });
      ...
      if (!deleted) return res.status(500).json({ message: "Failed to delete case" });
  ...
  app.post("/api/review-removal/cases/:id/save-draft", async (req, res) => {
    try {
      const userId = getUserId(req);
      const caseData = await storage.getReviewRemovalCase(req.params.id);
      if (!caseData) return res.status(404).json({ message: "Case not found" });
      ...
      if (!validStages.includes(stageKey)) {
        return res.status(400).json({ message: "Can only save submissions for challenge or arbitration stages" });
      }
  ...
  app.post("/api/review-removal/cases/:id/analyze", async (req, res) => {
    try {
      const userId = getUserId(req);
      const caseData = await storage.getReviewRemovalCase(req.params.id);
      if (!caseData) return res.status(404).json({ message: "Case not found" });
      ...
      const { updatedCase, analysis } = await runCaseAnalysis(storage, caseData, userId, additionalContext);
  ...
  app.post("/api/review-removal/cases/:id/revise", async (req, res) => {
    try {
      const userId = getUserId(req);
      const caseData = await storage.getReviewRemovalCase(req.params.id);
      if (!caseData) return res.status(404).json({ message: "Case not found" });
      ...
      if (!currentText || !userFeedback) {
        return res.status(400).json({ message: "Current text and feedback are required" });
      }
      ...
      res.json({ revisedText: stripMarkdown(result.revisedText || currentText) });
    } catch (error) {
  ...
  app.post("/api/review-removal/cases/:id/advance", async (req, res) => {
    try {
      const userId = getUserId(req);
      const caseData = await storage.getReviewRemovalCase(req.params.id);
      if (!caseData) return res.status(404).json({ message: "Case not found" });
      ...
      const stageOrder = ["analysis", "challenge_1", "challenge_2", "arbitration", "resolved"];
      const currentIndex = stageOrder.indexOf(caseData.stage);
      if (currentIndex === -1 || currentIndex >= stageOrder.length - 1) {
        return res.status(400).json({ message: "Cannot advance from current stage" });
      }
