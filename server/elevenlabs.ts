...
async function getCredentials(): Promise<string> {
  ...
  if (envApiKey) {
    // Log key length and first/last chars for debugging (never log full key)
    const keyLen = envApiKey.length;
    const keyPreview = keyLen > 8 ? `${envApiKey.slice(0, 4)}...${envApiKey.slice(-4)}` : '***';
    console.log(`[ElevenLabs] Using environment variable API key (length: ${keyLen}, preview: ${keyPreview})`);
    return envApiKey;
  }
  ...
  if (hostname && xReplitToken) {
    try {
      ...
      if (connectionSettings?.settings?.api_key) {
        console.log('[ElevenLabs] Using Replit Connectors API key');
        return connectionSettings.settings.api_key;
      }
    } catch (error) {
      console.log('[ElevenLabs] Replit Connectors not available');
    }
  }

  throw new Error('ELEVENLABS_API_KEY not configured. Please set it in your secrets or connect ElevenLabs.');
}
...
export async function generateSongWithTTS(lyrics: string, voiceId: string = 'Xb7hH8MSUJpSbSDYk0k2', musicStyle: string = 'pop'): Promise<Buffer> {
  const apiKey = await getCredentials();
...
Style: ${musicStyle}, upbeat, clear vocals, professional production`;
  ...
  console.log('[Song] Using Music API with prompt length:', musicPrompt.length);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.log('[Song] Music API error response:', response.status, errorText);
      throw new Error(`Music API error: ${response.status} - ${errorText}`);
    }
    
    const arrayBuffer = await response.arrayBuffer();
    console.log('[Song] Music API returned', arrayBuffer.byteLength, 'bytes');
    return Buffer.from(arrayBuffer);
  } catch (error: any) {
    console.log('[Song] Music API error:', error.message || error);
    throw error;
  }
}
